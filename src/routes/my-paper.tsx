...
interface Plan {
  ...
  chapters: Array<{ id: string; name: string; part: string; chapter_no: number }>
...
function MyPaper() {
        ...
        (data: {
          ...
          const offered =
            data.options.find((o) => o.board === data.profile.board && o.class === data.profile.class)
              ?.subjects ?? []
          const mine = offered.filter((s) => data.profile.subject_ids.includes(s.id))
          setSubjects(mine)
          const preferred = mine.find((s) => s.id === search.subject && s.has_content)
  ...
  async function loadPlan(subject: string, chapters: Array<string> | null) {
    ...
    try {
      ...
      if (chapters && chapters.length > 0) query.set('chapter_ids', chapters.join(','))
      ...
      setPlan(body)
      if (chapters === null) setChapterIds(body.chapters.map((c: { id: string }) => c.id))
    } finally {
  ...
  async function startTest() {
    ...
    try {
      ...
      if (!generated.ok) {
        setError(paper.message ?? (typeof paper.error === 'string' ? paper.error : 'Could not create the paper.'))
        return
      }
      if (paper.paperQuestions.length === 0) {
        setError('The question bank ran short for these chapters right now. Try other chapters or try again later.')
        return
      }
      ...
      if (!attempt.ok) {
        setError(typeof started.error === 'string' ? started.error : 'Could not start the test.')
        return
      }
  ...
  return (
    <div className="mx-auto max-w-3xl p-4 sm:p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-h1">Generate my question paper</h1>
      ...
      {subjects.length > 0 && withContent.length === 0 && (
        <Card>
          <CardContent className="text-body pt-6 text-muted-foreground">
            ...
            <a href="/profile-setup" className="text-primary underline-offset-4 hover:underline">
              Change subjects
            </a>
          </CardContent>
        </Card>
      ...
      {withContent.length > 1 && (
        <div className="mb-4 grid gap-3 sm:grid-cols-2" role="radiogroup" aria-label="Subject">
          {withContent.map((s) => (
            <SubjectCard
              key={s.id}
              name={s.name}
      ...
      {loadingPlan && !plan && <p className="text-body text-muted-foreground">Preparing your paper…</p>}
      ...
      {plan && (
        <div className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-h3">
                {plan.is_initial_assessment ? 'Your first assessment' : `${plan.subject_name} practice`}
              </CardTitle>
              <CardDescription>
            ...
            <CardContent>
              <dl className="text-body grid grid-cols-2 gap-x-4 gap-y-3 sm:grid-cols-4">
                ...
                <div>
                  <dt className="text-small text-muted-foreground">Questions</dt>
                  <dd>
                    {plan.total_questions} ({plan.total_marks} marks)
                  </dd>
                </div>
                <div>
                  <dt className="text-small text-muted-foreground">Difficulty</dt>
                  <dd>
                ...
                <div className="col-span-2 sm:col-span-4">
                  <dt className="text-small text-muted-foreground">Question types</dt>
          ...
          {allChapters.length > 1 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>We picked these for you. You can change them.</CardDescription>
              </CardHeader>
              <CardContent className="space-y-1">
                {allChapters.map((c) => (
                  <label key={c.id} className="text-small flex items-center gap-2">
                    <input
                      type="checkbox"
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-h3">What this paper covers</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <tbody>
                  {plan.concepts.map((c) => (
                    <tr key={c.concept_id} className="border-t">
                      ...
                      <td className="py-2 pr-2">{c.mastery_level ?? 'New'}</td>
                      <td className="py-2 text-right">{c.questions_planned}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <Button className="w-full" size="lg" disabled={starting || loadingPlan} onClick={() => void startTest()}>

...
function RemediationHub() {
  ...
  const [habitResult, setHabitResult] =
    useState<HabitDrillAttemptResult | null>(null)
  ...
  function refreshLists() {
    Promise.all([
      ...
      ([conceptData, habitData]: [
        ...
        setItems([
          ...conceptData.tasks.map((task): ListItem => ({
            kind: 'concept',
            task,
          })),
  ...
  async function submitHabitAttempt() {
    ...
    try {
      ...
      const response = await fetch(
        `/api/habit-drills/${habitDetail.id}/attempt`,
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
  ...
  return (
    <AppShell variant="student" active="drills">
              ...
              item.kind === 'concept' ? (
                <Card key={`concept-${item.task.id}`}>
                  ...
                  <CardContent>
                    <Button
                      variant={
                        item.task.status === 'completed' ? 'outline' : 'default'
                      }
